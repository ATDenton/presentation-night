...
Hit **Spin my topic** and a slot machine rattles through the list for a couple of seconds before
landing on yours. Because the board is synced live, the spin plays out on *both* screens at the
same time — so if one of us spins mid-call, the other watches it land in real time rather than
being told about it.
...
| | |
|---|---|
| **Spin my topic** | Picks a topic neither of us has ever had. Locks once spun. |
...
simultaneous spins can't collide or hand out the same topic. The reveal animation is driven by
`requestAnimationFrame` with a timer fallback, because browsers suspend animation frames in
background tabs — without it, a topic landing while your tab was hidden would freeze the
display on the wrong one.
